...
public class HomeController {
	   ...
	   @Autowired                                                  //autowires relationships between our collaborating beans
	    ...
	    public String home(Model model) {                         //
	        ...
	        int totalReportedCases = allStats.stream().mapToInt(stat -> stat.getLatestTotalCases()).sum();    //creates totalReportedCases
	        int totalNewCases = allStats.stream().mapToInt(stat -> stat.getDiffFromPrevDay()).sum();          //creates totalNewCases
